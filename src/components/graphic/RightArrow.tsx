...
export default function RightArrow({ width = 48, height = 48 }: ArrowProps) {
  // Each arrow width (approximate from coordinates)
  const arrowWidth = 8;
  const spacing = 0; // no extra spacing
  ...
  return (
    ...
    >
      ...
      {positions.map((x, i) => (
        <path
          key={i}
          d={`M${x - 3} 14 L${x + 5} 24 L${x - 3} 34`} // mirrored arrow coordinates
          stroke="white"
          strokeWidth="4"
          strokeLinecap="round"
